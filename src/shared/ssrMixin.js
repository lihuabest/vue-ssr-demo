  ...
  created() {
    if (TARGET === 'web') {
      if (window.__INITIAL_COMPONENTS_STATE__ && this.$router) {
        if (typeof window.__INITIAL_COMPONENTS_STATE__ === 'string') {
          window.__INITIAL_COMPONENTS_STATE__ = window.__INITIAL_COMPONENTS_STATE__.replace(/&quot;/g, '"');
          window.__INITIAL_COMPONENTS_STATE__ = JSON.parse(window.__INITIAL_COMPONENTS_STATE__);
        }
